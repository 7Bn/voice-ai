...
The system prompt is parameterized per practice via PracticeConfig.
It does NOT include the HIPAA disclosure — that's delivered verbatim as the
first utterance by the agent, not generated by the LLM.

The LLM's job:
  - Stay in character as the practice receptionist
  - Extract patient intent from conversation
  - Collect booking details (name, service, preferred time)
  - Recognize when to stop and escalate (state machine makes the decision,
    LLM surfaces the signal)
...
def build_system_prompt(
    ...
    return f"""You are {config.agent_name}, the AI receptionist for {practice_name}, a dental practice.
You represent {practice_name}. Your name is {config.agent_name}.
...
{services_text}
Only book appointments for services in this list. If a patient asks for something not listed,
say "I'll need to connect you with our team to help with that."
...
def _state_guidance(state: ConversationState) -> str:
    guidance = {
        ...
        ConversationState.COLLECT_DETAILS: (
            ...
            "Don't need an exact slot — approximate is fine."
        ),
